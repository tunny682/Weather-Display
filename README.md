...
- **No Pi taskbar:** setup creates your user LXDE autostart from the system one if missing (so it can be edited), then disables lxpanel there so the taskbar never starts; the launcher also runs `killall lxpanel` every 2 seconds as a backup. The app runs **fullscreen** by default (`config.json`). If the taskbar still appears, re-run `bash setup.sh` and reboot.
